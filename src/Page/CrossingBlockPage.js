...
const CrossingBlockPage = () => {
  const pageTitle = "Crossing Block";
  const { id } = useParams();
  const [pageState, setPageState] = useState(new State.StartState());
  const crossingBlockRepository = new CrossingBlockRepository(
    AppConfig.BaseURL
  );
  ...
  const loadData = async () => {
    try {
      const crossingBlock = await crossingBlockRepository.getCrossingBlockById(
        parseInt(id)
      );
